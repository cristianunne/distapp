...
import { getResumenVentasByFechaFromDB, getResumenVentasFromDB } from '../../databases/Entity/VentasEntity';

const VentasResumenDiariaScreen = ({route}) => {
    ...
    const [fechaVentas, setFechaVenta] = useState();


    const getVentasDB = async () => {

        const resumen_ventas = await getResumenVentasByFechaFromDB(1, fecha_venta);
        if (resumen_ventas.rows.length > 0) {

            setSubtotal(parseFloat(resumen_ventas.rows.item(0).subtotal != undefined ? resumen_ventas.rows.item(0).subtotal : 0));

            let desc_ = parseFloat(resumen_ventas.rows.item(0).descuentos != undefined ? resumen_ventas.rows.item(0).descuentos : 0) + 
            parseFloat(resumen_ventas.rows.item(0).descuento_general != undefined ? resumen_ventas.rows.item(0).descuento_general : 0);
          
        
            setDescuentos(desc_ != undefined ? desc_ : 0);
            setTotal(parseFloat(resumen_ventas.rows.item(0).total != undefined ? resumen_ventas.rows.item(0).total  : 0));

        }

        const resumen_ventas_cc = await getResumenVentasByFechaFromDB(0, fecha_venta);
        if (resumen_ventas.rows.length > 0) {

            setSubtotalcc(parseFloat(resumen_ventas_cc.rows.item(0).subtotal != undefined ? resumen_ventas_cc.rows.item(0).subtotal : 0));

            let desc_cc = parseFloat(resumen_ventas_cc.rows.item(0).descuentos != undefined ? resumen_ventas_cc.rows.item(0).descuentos : 0) + 
            parseFloat(resumen_ventas_cc.rows.item(0).descuento_general != undefined ? resumen_ventas_cc.rows.item(0).descuento_general : 0);

            setDescuentoscc(desc_cc);
            setTotalcc(parseFloat(resumen_ventas_cc.rows.item(0).total != undefined ? resumen_ventas_cc.rows.item(0).total : 0));
      ...
      <View style={styles.box_content}>
          <View style={styles.box_text_content}>
            ...
            <SafeAreaView>
                ...
                <Text style={styles.text_fecha}>Fecha: {fecha_venta}</Text>
...
export default VentasResumenDiariaScreen
